...
//        $this->Cell(0, 10, 'Page ' . $this->PageNo() . '/{nb}', 0, 0, 'C');
    ...
    function TablaSimple($header) {        
        ...
        foreach ($objCreditImpl->getCreditOnlyBetweenDate($_POST['txbFechaInicio'], $_POST['txbFechaFin']) as $valorCredit) {     
            ...
            $DiasMora = $objCreditImpl->getDaysMora($valorCredit->getCode());

            $this->Cell(12, 5, $valorCredit->getCode(), 0, 0, 'C');
            $this->Cell(14, 5, $valorCredit->getCodeBill(), 0, 0, 'C'); 
            
            $objClientImpl = new ClientImpl();
            $this->Cell(50, 5, $valorCredit->getCodeClient().' - '. $objClientImpl->getNameClient($valorCredit->getCodeClient()), 0);
            ...
            $this->Cell(10, 5, floor($DiasMora), 0, 0, 'R');
            $this->Ln(6);
